...
/* Лабораторная работа №3
	...
	* ИВБ-3-14
	* Панасенко Андрей
Даны две матрицы разного размера. Для той из матриц, в которой меньше среднее арифметическое положительных элементов, проверить наличие отрицательных элементов в каждой строке */
...
static double
__findMinElement(double **pMatrix, int rows, int cols);
...
main(int argc, char **argv)
{
	...
	/** Find max element */
	double avg1 = __findMaxElement(matrix1, mRows1, mCols1);
	double avg2 = __findMaxElement(matrix2, mRows2, mCols2);
	if (avg1 > avg2) {
		fprintf(stdout, "Output matrix N1:\n");
		__outputMaxElements(matrix1, mRows1, mCols1);
		__findMinElement(matrix2, mRows2, mCols2);
	} else {
		fprintf(stdout, "Output matrix N2:\n");
		__outputMaxElements(matrix2, mRows2, mCols2);
		__findMinElement(matrix1 , mRows1, mCols1);
	}
...
__findMaxElement(double **pMatrix, int rows, int cols)
{
	...
	return result == 0 || result == 0 ? 0 : result / n ;
}
